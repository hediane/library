    ...
    node
        {
            ...
            stage('Publish') 
                {  //sh "${config.dockerComposeLocation} -f ${config.dockerComposeElasticDestintination} build "
                    ...
                    //echo "${config.dockerfileLocation}",
                    //sh "dotnet pack ${config.ProjectName}"
                    sh "dotnet nuget push /var/jenkins_home/workspace/Back_main/source/DevOpsProject/bin/Debug/aosora.1.0.0.nupkg -s http://192.168.56.115:8081/repository/AosoBack/ -k 5f15c27d-1f8a-3baa-a136-cd624ba7c9b7 "
                    //sh 'dotnet pack '

        ...
        dir("/var/lib/jenkins/workspace/Back_main/source/DevOpsProject")
             {   stage('Packing') 
                ...
                //sh "${config.dockerComposeLocation} -f ${config.dockerComposeNexusDestination} up -d"
                   ...
                   sh "dotnet pack  /var/lib/jenkins/workspace/Back_main/source/DevOpsProject/bin/Release/net5.0/ --no-build --output nupkgs"
            ...
            stage('Publish') 
                { 
                 sh "dotnet nuget push /var/lib/jenkins/workspace/Back_main/source/DevOpsProject/bin/Debug/aosora.1.0.0.nupkg -s http://192.168.56.115:8081/repository/nuget-hosted/ -k 5f15c27d-1f8a-3baa-a136-cd624ba7c9b7 "
